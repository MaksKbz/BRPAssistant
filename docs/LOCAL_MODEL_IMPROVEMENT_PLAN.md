...
Статус: добавлен typed `BrpReadOnlyToolRegistry` с тестами. Поддерживаются только read-only domain operations; подключение конкретного LLM tool-call adapter остаётся отдельной интеграцией.
